    ...
    for (i = 0; i < n2; i++) {
        ...
        for (j = 0; j < n1; j++) {
            if (process[i] <= block[j]) {
                diff[count] = block[j] - process[i];  // Calculate remaining space
                mem[count] = j;                         // Store memory block index
                count++;
            }
        }
        ...
        if (count == 0) {
            ...
            block[mem[0]] -= process[i];  // Update memory block size after allocation
            printf("%d\t\t%d\t\t%d\n", i + 1, process[i], mem[0] + 1);
        }
    }
